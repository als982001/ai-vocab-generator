...
- **대량 데이터 처리**: 수백 개의 단어 카드를 필터링/정렬할 때 매 렌더링마다 재계산하면 성능 저하
- **의존성 관리**: 정렬 옵션이나 필터 조건이 변경될 때만 재계산하도록 최적화
