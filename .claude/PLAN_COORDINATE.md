...
- **상호작용 (Interaction)**:
  - `hoveredWordId` 상태(State)를 상위 컨테이너로 끌어올려(Lift State Up), 이미지 박스와 단어 리스트가 상태를 공유하도록 함.
...
4. 반대로 이미지의 박스에 마우스를 올리면 시각적 피드백이 오는가?
