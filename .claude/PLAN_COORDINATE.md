...
- [x] **반응형 처리**: CSS `%` 단위 사용으로 자동 대응 ✅ 완료
- [x] **스타일링**: `cursor-pointer`, `transition-all duration-200` 적용 ✅ 완료

### Step 5: 이미지의 박스 클릭하면 화면 스크롤

- [x] **스크롤 추가**: 우측 단어들에 스크롤이 적용 안 되어 있는데 스크롤되게 수정
- [x] **이미지 단어 클릭 시 스크롤**: 단어를 클릭하면 우측 단어 리스트가 스크롤되어서 해당 단어를 가장 위에 보여주기
